...
@SuppressWarnings("deprecation") public class CalculateMapper extends MapReduceBase
  ...
  public void map(LongWritable key, Text value, OutputCollector<Text, Text> output,
    ...
    String auth =
        Text.decode(value.getBytes(), pageTabIndex + 1, authTabIndex - (pageTabIndex + 1));
    String hub = Text.decode(value.getBytes(), authTabIndex + 1, hubTabIndex - (authTabIndex + 1));

    String[] linksInOut =
        Text.decode(value.getBytes(), hubTabIndex + 1, value.getLength() - (hubTabIndex + 1))
            .split("\t");
    if (linksInOut.length < 2) return;
