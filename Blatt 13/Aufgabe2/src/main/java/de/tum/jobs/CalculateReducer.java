...
@SuppressWarnings("deprecation") public class CalculateReducer extends MapReduceBase
    implements Reducer<Text, Text, Text, Text> {

  public void reduce(Text page, Iterator<Text> values, OutputCollector<Text, Text> out,
    ...
    if (linksInOut.split("\t").length < 2) return;

    auth = auth / Math.sqrt(normAuth);
    hub = hub / Math.sqrt(normHub);
    if (Double.isNaN(auth)) auth = 0.0;
    if (Double.isNaN(hub)) hub = 0.0;
